...
class BiliDownloaderGUI(QMainWindow):
    ...
    def create_options_tab(self):
        ...
        self.quality_combo.addItem("8K 超高清 (需要登录)", 127)
        self.quality_combo.addItem("4K 杜比视界 (需要登录)", 126)
        self.quality_combo.addItem("4K 超清 (需要登录)", 120)
        self.quality_combo.addItem("1080P 60帧 (需要登录)", 116)
        self.quality_combo.addItem("1080P+ 高码率 (需要登录)", 112)
        self.quality_combo.addItem("1080P 高清 (需要登录)", 80)
        ...
        # 添加清晰度说明
        note_label = QLabel("注意：标注'需要登录'的清晰度需要扫码登录后才能下载")
        note_label.setStyleSheet("color: #e74c3c; font-size: 11px; font-style: italic;")
        quality_layout.addWidget(note_label)
